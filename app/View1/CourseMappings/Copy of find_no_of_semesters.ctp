<!-- Edit line numbers 50, 82 and 86 -->
...
for ($i=1; $i<= $programSemesters; $i++) {
		...
		echo "<div>";
		echo $this->Form->create('Semester'.$i,array('url' => array('controller' => 'CourseMappings', 'action' => 'index')));
		...
		echo $this->Form->input('Month&nbsp;Year', array('name'=>'MonthYear','id'=>'MonthYear'.$i,'label' => "Month&nbsp;Year<span class='ash'>*</span>", 'type' => 'select', 'options'=>$monthYears, 'default'=>$selDDMonthYear));
	...
	if(isset($arr[$i])){
		...
		$mandatory = $arr[$i][$key]['mandatory'];
			...
			$QueryMode = "A";
			if($arr[$i][$key]['course_mapping_id']){$QueryMode = "E";}
			...
			echo $this->Form->input('CT', array('label' => false, 'class' => 'col-sm-4', 'empty' => __("----- Course Type-----"), 'options' => $course_mode, 'default'=>$arr[$i][$key]['course_mode_id'],'onchange'=>'toggleCT('.$i.',this.value,'.$key.');','name'=>"Semester".$i."CT".$key,'id'=>"Semester".$i."CT".$key)); //, $disabled
			
			$spanVar = "spanSemester".$i."CourseId".$key;
			echo "<span id='$spanVar'>";			
			echo $this->Form->input('course_id', array('label' => false,'class' => 'col-sm-6 chosen', 'empty' => __("----- Select Course-----"), 'options' => $allCourses, 
									'default'=>$arr[$i][$key]['course_id'], 'multiple' => 'multiple', 'name'=>"Semester".$i."CourseId".$key,'id'=>"Semester".$i."CourseId".$key)); //, $disabled
			//echo $this->Form->input('Mandatory', array('type'=>'checkbox', 'label' => false, 'name'=>"Mandatory".$i."Course".$key,'id'=>"Mandatory".$i."Course".$key, 'default'=>$mandatory));									
			echo "</span>";
		...
		//if($this->Html->checkPathAccesstopath('CourseMappings/add','',$authUser['id'])){
		//,$btnDisabled
			echo $this->Form->button('<i class="ace-icon fa fa-check bigger-110"></i>'.__('Submit'),array('style'=>'float:right;','type'=>'button','name'=>'Save'.$i,'id'=>'Save'.$i,'value'=>'Save','onclick'=>"saveCourseMapping($batchId,$programId,$i);",'class'=>'btn'));
		//}
		echo "</div>";
		echo $this->Form->end();
		echo "</div>";
...
<?php for($i=1; $i<=$programSemesters; $i++) { ?>
	$("#Semester<?php echo $i; ?>btn").click(function() { 
		...
		if(cnt > 0){			
			...
			newtag+="</select>";
			newtag+="</span>&nbsp;&nbsp;";
			//newtag+="<span style='margin-top:10px;'><input type='checkbox' name='Mandatory<?php echo $i; ?>Course"+(parseInt(i)+1)+"' id='Mandatory<?php echo $i; ?>Course"+(parseInt(i)+1)+"'></span>";
			newtag+="</div>";
			}			
