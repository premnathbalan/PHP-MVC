...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="margin-top:10px;">
	...
	<tbody>
	<?php $i =1; foreach ($markArray as $student_id => $studentArray) { 
	//pr($studentArray);
	?>
	<tr class=" gradeX">
		...
		<td>
				<?php
					echo $this->Form->input('cae_old_marks', array('type'=> 'hidden','id'=>'CaeOldMark'.$i, 'default'=>$cae_results[$student_id]['marks'], 'style'=>'position:relative;margin-top:-14px;width:50px;border-color:#000;color:#000;padding-left:10px;', 'label'=>false, 'name' => 'data[Practical][cae_old_marks]['.$student_id.']'));
		...
		<td>	
			...
			echo $this->Form->input('marks', array('type'=> 'text','id'=>'Arrear'.$i, 'class'=>'dummy', 'style'=>'position:relative;margin-top:-14px;width:50px;border-color:#000;color:#000;padding-left:10px;','default'=>$marks, 'label'=>false, 'name' => 'data[Practical][marks]['.$student_id.']', 'onblur'=>"ArrearPracESEEntry($i,this.value,$student_id,$exam_month_year_id);"));
			?>
		</td>
	</tr>
	<?php $i++;} ?>
