...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="margin-top:10px;">
			...
			<tbody>
				<?php 
				//pr($caeMarks);
				if(isset($caeMarks) && count($caeMarks) > 0) {
				if (isset($result)) { 
				//pr($caeMarks);
				$j=1;
				...
				<tr class=" gradeX">
					...
					<td><?php 
								...
								echo $this->Form->input('id', array('type'=>'hidden', 'label' => false, 'default' => $temp['id'], 'name'=>'data[CaePractical][id][]'));
								echo $temp['marks'];
						?></td>
				</tr>
				  <?php
		...
		<?php
		if($approvalStatus) {
			echo $this->Form->button('<i class="ace-icon fa fa-check bigger-110"></i>'.__('Approved'),array('type'=>'button','name'=>'submit','value'=>'submit','class'=>'btn js-cae-practical-approve', 'disabled'));
		}
		else {
			echo $this->Form->button('<i class="ace-icon fa fa-check bigger-110"></i>'.__('Approve'),array('type'=>'button','name'=>'submit','value'=>'submit','class'=>'btn js-cae-practical-approve'));			
		}
		echo $this->Form->end();
		?>
