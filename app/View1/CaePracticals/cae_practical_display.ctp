...
?>
	...
	<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="margin-top:10px;">
			...
			foreach ($courseMappingArray as $courseMapping) {
				$caePracticalArray = $courseMapping['CaePractical'];
				$i=1;
				foreach ($caePracticalArray as $key => $caePracticalDetails) {
					$caeId = $caePracticalDetails['id'];
					$marks_status = $caePracticalDetails['marks_status'];
				if (trim($caePracticalDetails['marks_status']) == "Entered" && ($caePracticalDetails['approval_status'] == 1)) { 
					$bgStyle = 'ovelShapBg1';
				}
				if (trim($caePracticalDetails['marks_status']) == "Entered" && ($caePracticalDetails['approval_status'] == 0)) { 
					$bgStyle = 'ovelShapBg2';
				}
				if ($caePracticalDetails['add_status'] == 1) {
						$markStatusMethod="editStudents";
				}
				else if ($caePracticalDetails['add_status'] == 0) {
					$markStatusMethod="getStudents";
				}
				if ($caePracticalDetails['approval_status']) {
				...
				<tr class='gradeX'>
					...
					<td>
							...
							if ($marks_status == "Not Entered") {
								if($this->Html->checkPathAccesstopath('CaePracticals/addCaePracticalMarks','',$authUser['id'])){
									echo $this->Html->link("<i class='fa fa-plus fa-lg'></i>", array("controller"=>"CaePracticals",
									'action' => 'addCaePracticalMarks', $batch_id, $academic_id, $program_id, $caeId, $i, $month_year_id), 
									array('title'=>'Add','escape' => false))."&nbsp;&nbsp;";
								}
							}
							if($this->Html->checkPathAccesstopath('CaePracticals/editCaePractical','',$authUser['id'])){
								echo $this->Html->link("<i class='fa fa-pencil fa-lg'></i>", array("controller"=>"CaePracticals",
								'action' => 'editCaePractical', $batch_id, $academic_id, $program_id, $caeId, $i, $month_year_id), 
								array('title'=>'Edit','escape' => false))."&nbsp;&nbsp;";
							}
							if($this->Html->checkPathAccesstopath('CaePracticals/viewCae','',$authUser['id'])){
								echo $this->Html->link("<i class='fa fa-eye fa-lg'></i>", array("controller"=>"CaePracticals",
								'action' => 'viewCae', $batch_id, $academic_id, $program_id, $caeId, $i, $month_year_id), 
					...
					<td>Practical</td>
					<td><?php echo $caePracticalDetails['assessment_type']; ?></td>
					...
					<td><?php echo $i; ?></td>
					<td><?php echo $caePracticalDetails['marks']; ?></td>
					<td><?php echo $caePracticalDetails['marks_status']; ?></td>
					<td><?php echo $caePracticalDetails['approval_status']; ?></td>
					<td>Lecturer</td>
				</tr>
				<?php
				$i++;
