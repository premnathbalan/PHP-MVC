...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="margin-top:10px;">
	...
	<?php foreach ($results as $key => $result) {
		...
		$new_total = $result['RevaluationExam']['total_marks'];
		if ($revaluation_mark>=$ese_from && $revaluation_mark<=$ese_to) {
			$ese_bool = true;
		}
		else {
			$ese_bool = false;
		}
		if ($new_total>=$total_from && $new_total<=$total_to) {
			$total_bool = true;
		}
		else {
			$total_bool = false;
		...
		<tr class=" gradeX">
			...
			<td align='center'>
				...
				<?php echo $this->Form->input('old_ese_mark', array('type'=>'hidden','label'=>false, 'value'=>$result['Revaluation']['ese_marks'], 'name'=>'data[RevaluationExam][oldEseMark][]')); ?>
			</td>
			<td align='center'>
				<?php 
