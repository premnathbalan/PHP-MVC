		...
		if (isset($unCommonPassArray[$i]) && count($unCommonPassArray[$i]) > 0) {
				...
				if (isset($ucpeq)) {
				 ...
				 //echo $ucpeq[$uncommon_pass_cm_id];
					echo $this->Form->input('uncommon_pass_cm_id_checkbox', array('type'=>'checkbox', 'class'=>'StudentUncommonPassCmIdCheckbox', 'name'=>'data[MapStudents][UnCommonPassCheckbox]['.$uncommon_pass_cm_id.']', 'label'=>false, $checked))." ".$ucp_course_code." ";
					//pr($transferToUnCommonArray[$i]);
					//removed and placed at last columnu
					//echo $this->Form->input('uncommon_pass_cm_id', array('type'=>'hidden', 'label'=>false, 'value'=>$uncommon_pass_cm_id, 'style'=>'margin-left:5px;', 'name'=>'data[MapStudents][UnCommonPass]['.$uncommon_pass_cm_id.']'));
					echo "</br>";
				}
				if (isset($NewMonthYearId[$i-1]) && $NewMonthYearId[$i-1] > 0) {
					$defaultSem = $NewMonthYearId; 
				...
				echo $this->Form->input('ucp_my_id', array('type'=>'select', 'name'=>'data[MapStudents][ucp_my_id]', 'options'=>$month_years, 'empty'=>'--Select--', 'label'=>false, 'default'=>$defaultSem));
				$defaultSem="";
		}
		?></td>
