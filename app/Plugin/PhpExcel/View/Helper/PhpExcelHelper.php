...
class PhpExcelHelper extends AppHelper {
    ...
    public function addTableHeader($data, $params = array()) {
        ...
        if (isset($params['merge']))
			$this->_xls->getActiveSheet()->mergeCells($params['merge']);
		//row height
		if (isset($params['row_height']))
		$this->_xls->getRowDimension('1') -> setRowHeight($params['row_height']); // Set Row Height For 1
	//row height
		if (isset($params['col_width']))
		$this->_xls->getRowDimension('1') -> setRowHeight($params['col_width']); // Set Row Height For 1
		// alignment
        if (isset($params['alignment_h']))
			$this->_xls->getActiveSheet()->getStyle($this->_row)->getAlignment()->setHorizontal($params['alignment_h']);
	     if (isset($params['alignment_v']))
			$this->_xls->getActiveSheet()->getStyle($this->_row)->getAlignment()->setVertical($params['alignment_v']);
        ...
        foreach ($data as $d) {		
			...
			// merge
			if (isset($d['merge'])){			
     			$this->_xls->getActiveSheet()->mergeCells($d['merge'][0]);				
			//	$offset = $offset+$d['merge'][1];
			}			
