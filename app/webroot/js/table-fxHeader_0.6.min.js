...
(function() {
    ...
    this.fxheader = function() {
        if (!l) {
            ...
            for (var a = 0; a < e.length; a++) {
                ...
                var d = i.childNodes,
                    c;
                for (c = 0; c < d.length; c++) i.removeChild(d[c]);
                ...
                for (c = d = 0; c < e[a].noOfRows; c++) {
                    h = f.rows[c].cells;
                    var m, n = b.rows[c].cells,
                        j;
                    if (k) {
                        m = k.rows[c].cells;
                        for (j = 0; j < h.length; j++) h[j].style.width = m[j].style.width = n[j].offsetWidth - 3 + "px"
                    } else
                        for (j = 0; j < h.length; j++) h[j].style.width = n[j].offsetWidth - 3 + "px";
                    d += b.rows[c].offsetHeight
                }
                b.style.marginTop = "-" + d + "px";
                f = e[a].sheight;
                if (b.offsetHeight < f) f = b.offsetHeight + 18;
                h = 0;
                if (e[a].noOfCols > 0) {
                    for (c = 0; c < e[a].noOfCols; c++) h += b.rows[0].cells[c].offsetWidth;
    ...
    this.fxheaderInit = function(a, b, g, i) {
        ...
        if (!d.swidth || d.swidth.length == 0) {
            ...
            if (!d.swidth) d.swidth = "100%";
            if (d.swidth.indexOf("%") == -1) d.swidth = parseInt(d.swidth)
        }
        d.noOfRows = g;
        d.noOfCols = i;
        if (!ge$(a + ":scroller")) {
            ...
            c.id = a + ":scroller";
            c.style.cssText = "height:auto;overflow-x:auto;overflow-y:auto;width:auto;font-size:13px;font-family:Arial";
            ...
            f.innerHTML = '<div id="' + a + ':scroller:fx" style="text-align:left;position:relative;width:9999px;padding:0px;margin-left:0px;"><font size="3" color="red"></font></div>';
            var h = null;
            if (i > 0) {
