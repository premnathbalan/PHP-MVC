...
(function($ , undefined) {
	...
	.on(ace.click_event+'.ace.menu', '.sidebar-collapse', function(e){ //alert("SIDE BAR COLLAPSE/EXPAND BUTTON....-PALANI");
		if(($('.menu-icon').width()) == 30){ 
			totalWidth = ($('.display').width()+170);
			$('.display').css({"width": totalWidth});
		}else{
			totalWidth = ($('.display').width()-170);
			$('.display').css({ "width": totalWidth});
		}
