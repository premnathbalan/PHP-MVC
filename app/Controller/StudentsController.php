...
class StudentsController extends AppController {
	...
	public function add(){ 
		if ($this->request->is('post')) { //pr($this->data);
	...
	public function dcSearch(){
		...
		if($this->request->is('post')) {
			if(isset($this->request->data['submit']) == 'PRINT'){
				...
				//End
				$results = $this->Student->find("all", $res); */
				$results = $this->getStudentData($batchId, $programId, $examMonth, $frm_register_no, $to_register_no, "-");
				$this->set('results', $results);
				//pr($results);
			...
			foreach ($stuArray as $key => $value) {
				...
				SWITCH ($course_type_id) {
					...
					CASE 3:
						...
						if (isset($result_practical[0]['EsePractical'][0]['Practical'][0]['marks']))
		...
		$programResult = $this->CourseMapping->find('all', array(
				...
				'contain' => array(
						...
						'Program' => array(
								...
								'Faculty' => array(
										'fields' => array('Faculty.faculty_name','Faculty.faculty_name_tamil')
								)
						),
						'StudentMark' => array(
								'fields'=>array('StudentMark.month_year_id'),
								'limit'=>1,
		...
		foreach ($stuResults as $key => $stuArray) {
			...
			if ($csmCount == $passCount && $failCount == 0) {
				...
				array(
						...
						"faculty_tamil" => $array['faculty_tamil'],
	...
	public function cgpaList($batchId = null,$Academic = null,$programId = null, $monthYearId=null, $withheldType = null,$withheldVal = null,$printMode = null) {
		
		$this->set('batchId', $batchId);
		$this->set('Academic', $Academic);
		$this->set('programId', $programId);
		$this->set('monthYearId', $monthYearId);
		$this->set('withheldType', $withheldType);
		$this->set('withheldVal', $withheldVal);
		$this->set('printMode', $printMode);
		...
		//$stuCond['Student.registration_number'] = '3525076';
		...
		//$csmResult = $this->Student->enrolledCoursesForABatchAndProgram($batchId, $programId);
		...
		$finalArray = $this->processCgpaReport($results, $monthYearId, $batchId, $programId);
		$this->set('finalArray', $finalArray);
		
		$this->layout= false;
		if($printMode == 'Dept Excel'){
			$this->excelCGPA($finalArray,$withheldType, $totalCourses);
		}		
		if($printMode == 'Excel'){
			return $finalArray;
		}
	...
	public function transferCourses($reg_number=NULL) {
		...
		if (isset($results[0]['Student']['parent_id']) && ($results[0]['Student']['parent_id']) > 0) {
			/**** Student transfer "TRANSFER id = 3 dtabase monthyearid only 
			 * manipulate the semesters here
			 * ****/
			...
			foreach ($results as $key => $result) {
				...
				$tmp_month_year_id = $result['Student']['month_year_id'];
				
				//Semester manipulation goes here
				$semester_result = $this->getSemesterIdFromMonthYear($new_batch_id, $new_program_id, $tmp_month_year_id);
				$transferred_semester_id = $semester_result['CourseMapping']['semester_id'];
				//echo $semester_id;
				 
				//$transferred_semester_id = $result['Student']['month_year_id'];
			...
			foreach ($oldCourseStudentMapping as $key => $value) {
				...
				//echo $semester_id."</br>"; 
				if ($semester_id < $transferred_semester_id) {
			...
			//echo "new_my_id:".$transferred_semester_id;
			//echo "semester_id : ".$semester_id;
			
			
			$semester_to_process_data = $transferred_semester_id-1;
			...
			$previous_month_years = $transferred_semester_id-1;
			$tmpArray = array();
		   ...
		   /*  for($i=1; $i<=$semester_to_process_data; $i++) {
				pr($commonPassArray[$i]);
				if (isset($commonFailArray[$i])) {
				pr($commonFailArray[$i]);
				} else {
					echo "No Common Fail";
				}
			} */
	...
	public function arrearEditIndividualMarks() {
		...
		SWITCH ($course_type_id) {
			CASE 1:
				...
				$new_ese_mark = $_POST['data']['ese_new_marks'][$cm_id];
				//echo $old_ese_mark." ".$new_ese_mark;
				if ($assessment_type == 'cae') {
					$new_cae_mark = $this->manipulateTheoryCae($_POST, $cae_model);
				}
	...
	//pr($_POST);
		...
		else if ($course_type_id == 2 || $course_type_id == 6 ) {
			$cae_model = "InternalPractical";
			$ese_model = "Practical";
...
/* 		$min_cae_mark = $course_details[0]['Course']['min_cae_mark'];
	...
	public function insertMark ($mark, $model, $cm_id, $student_id, $month_year_id, $status, $grade, $grade_point, $course_type_id, $cid, $assessment_type, $type) {
		...
		//echo $course_type_id." ".$cid." ".$model;
		
		//$cIdArray = $this->getCourseIdFromCmId($cm_id);
		
		//$cid = $cIdArray[0]['Course']['id'];
		//echo $cid; die; 
		...
		$conditions = array();
		if (($course_type_id == 2 || $course_type_id == 6) && $assessment_type=='cae') {
			$conditions["$model".".cae_practical_id"]=$cid;
		} else if (($course_type_id == 2 || $course_type_id == 6) && $assessment_type=='ese') {
		...
		else if (($course_type_id == 2 || $course_type_id == 6) && $assessment_type=='cae') {
  ...
  public function rankSearch() {
  	if ($this->request->is('post')) {
		...
		foreach ($programResult as $key => $pgmResult) { //pr($pgmResult);
			...
			$batch_period = $pgmResult['Batch']['batch_from']."-".$pgmResult['Batch']['batch_to'];
			//$this->set(compact('batch_period'));
			$program_id = $pgmResult['CourseMapping']['program_id'];
			$batch_id = $pgmResult['CourseMapping']['batch_id'];
			$cm_id = $pgmResult[0]['CMId'];
			$pgm_short_code = $pgmResult['Program']['short_code'];
			$academic_short_code = $pgmResult['Program']['Academic']['short_code'];
			$last_month_year_id = $pgmResult['StudentMark'][0]['month_year_id'];
			...
			$array['semester'] = $pgmResult[0]['semester_id'];
			$array['max_marks'] = $pgmResult[0]['max_marks'];
			$array['faculty'] = $pgmResult['Program']['Faculty']['faculty_name'];
			$array['faculty_tamil'] = $pgmResult['Program']['Faculty']['faculty_name_tamil'];
			$array['month_year_id'] = $last_month_year_id;
  ...
  public function markList($batchId = null,$Academic = null,$programId = null,$withheldType = null,$withheldVal = null,$printMode = null) {
  	...
  	$programResult = $this->CourseMapping->find('all', array(
  			...
  			'fields' => array('DISTINCT CourseMapping.program_id', 'CourseMapping.batch_id', 'CourseMapping.semester_id',
  					'GROUP_CONCAT(CourseMapping.id) AS CMId', 'MAX(CourseMapping.semester_id) AS semester_id'),
  			'contain' => array(
  					'Course' => array(
  							'fields' => array('SUM(Course.course_max_marks) as max_marks'),
  			...
  			'group'=>array('CourseMapping.batch_id','CourseMapping.program_id','CourseMapping.semester_id')
  	));
  	//pr($programResult);
  	
  	//Get students
  	$studentArray = $this->Student->getStudentsInfo($batchId, $programId);
  //	pr($studentArray);
  	foreach ($studentArray as $student_id => $stuArray) {
  		$csmArray = $this->CourseStudentMapping->getEnrolledCourses($student_id);
  		//pr($csmArray);
  		$finalArray[$student_id]['csm'] = $csmArray[$student_id];
  		
  	}
  	foreach ($programResult as $key => $semArray) {
  		//pr($semArray);
  		//$tmpCourses = $semArray[]
  		//$cmArray[$key+1]
  	}
  ...
  public function dataSearch() {
  	...
  	if($this->request->is('post')) {
  		...
  		$results = $this->Student->find('all', array(
  			'conditions'=>array('Student.discontinued_status'=>0, 'Student.batch_id'=>$batchId, 
  					'Student.program_id'=>$programId
  			),
  			'fields'=>array('Student.registration_number', 'Student.roll_number', 'Student.father_name', 'Student.name',
  					'Student.mother_name', 'Student.birth_date', 'Student.gender', 'Student.nationality', 'Student.aadhar',
  					'Student.religion', 'Student.community', 'Student.phone_number', 'Student.email', 'Student.address',
  			),
  			'recursive'=>0,
  			'contain'=>false
  		));
		...
		$sheet->setCellValue("F1", "MOTHER NAME");
		...
		$sheet->setCellValue("L1", "RELIGION");
		$sheet->setCellValue("M1", "COMMUNITY");
		$sheet->setCellValue("N1", "AADHAR");
		
		$i=2;
		foreach ($results as $key => $value) {
			$aadhar = "";
			if (isset($value['Student']['aadhar'])) $aadhar = $value['Student']['aadhar'];
			$sheet->getRowDimension($i)->setRowHeight('18');
			$sheet->setCellValue('A'.$i, $value['Student']['registration_number']);
			$sheet->setCellValue('B'.$i, $value['Student']['roll_number']);
			$sheet->setCellValue('C'.$i, $value['Student']['name']);
			$sheet->setCellValue('D'.$i, $value['Student']['gender']);
			$sheet->setCellValue('E'.$i, $value['Student']['father_name']);
			$sheet->setCellValue('F'.$i, $value['Student']['mother_name']);
			$sheet->setCellValue('G'.$i, date("d-m-Y", strtotime($value['Student']['birth_date'])));
			$sheet->setCellValue('H'.$i, $value['Student']['address']);
			$sheet->setCellValue('I'.$i, $value['Student']['phone_number']);
			$sheet->setCellValue('J'.$i, $value['Student']['email']);
			$sheet->setCellValue('K'.$i, $value['Student']['nationality']);
			$sheet->setCellValue('L'.$i, $value['Student']['religion']);
			$sheet->setCellValue('M'.$i, $value['Student']['community']);
			$sheet->setCellValue('N'.$i, $aadhar);
			$i++;
		}
	
		$pgmDetails = $this->Program->getProgram($programId);
		
		$download_filename="Student_Data_".$this->Batch->getBatch($batchId)."_".$pgmDetails['academic_short_code']."_".$pgmDetails['short_code']."_".date('d-M-Y h:i:s');
		header("Content-Type: application/vnd.ms-excel");
  	...
  	if (!$access_result) {
  		if($this->request->is('post')) {
  			...
  			if(isset($this->request->data['submit']) == 'PRINT'){
  				...
  				if($this->request->data['markSheet']['type_of_cert']){
  					$type_of_cert = $this->request->data['markSheet']['type_of_cert'];
  					$issue_date = $this->request->data['markSheet']['issuing_date'];
  					$pgm_completion_status = $this->request->data['markSheet']['pgm_status'];
  					if (isset($this->request->data['discontinued_date'])) {
  						$discontinued_date = $this->request->data['discontinued_date'];
  					}
