				...
				foreach ($csmCourseArray as $student_id => $csm_status) {  
					if ($csm_status == 1) {
						...
						if ($this->CourseStudentMapping->hasAny($conditions)){ 
						 ...
						 // 	$this->CourseStudentMapping->updateAll(array(
							// 'CourseStudentMapping.modified' => "'$cur_date'",
							// 'CourseStudentMapping.indicator' => $indicator,
							// 'CourseStudentMapping.modified_by' => $this->Auth->user('id')
							// ),
							// array('CourseStudentMapping.student_id' =>$student_id,
							// 	'CourseStudentMapping.course_mapping_id' =>$cmId[trim($course_code)],
							// 	'CourseStudentMapping.course_id' =>$courseId[trim($course_code)])
							// );
							$bool = true; 
