 ...
 */
	...
	public function batchwiseReport($batch_id, $month_year_id, $print=NULL, $program_id) {
		...
		if ($program_id=='-') $cmaCond['CourseMapping.program_id >'] = 0;
		else $cmaCond['CourseMapping.program_id'] = $program_id;
		...
		foreach ($course_types as $course_type_id => $course_type_value) {
			switch ($course_type_id) {
				CASE 1:
					...
					$cm_results = $this->theoryCourseMappingArray($batch_id, $courseTypeIdArray, $program_id, $month_year_id);
					//pr($cm_results);
				...
				CASE 2:
					...
					$cm_results = $this->practicalCourseMappingArray($batch_id, $courseTypeIdArray, $program_id, $month_year_id);
				...
				CASE 4:
					...
					$cm_results = $this->projectCourseMappingArray($batch_id, $courseTypeIdArray, $program_id, $month_year_id);
					//echo count($cm_results);
					//pr($cm_results);
					
					$finalArray = $this->processProjectResults($cm_results, $course_details, $month_year_id);
				...
				CASE 5:
					...
					$cm_results = $this->profTrainingCourseMappingArray($batch_id, $courseTypeIdArray, $program_id, $month_year_id);
					//$cm_results = $this->profTrainingCourseMappingArray(2, $courseTypeIdArray, 7, 4);
					//echo count($cm_results);
					//pr($cm_results);
	...
	public function getReport($month_year_id = null, $batch_id = null, $print=NULL) {
		$batchesArray = array();
		/* $ttResult = $this->Timetable->find('all', array(
				'conditions' => array("Timetable.month_year_id"=>$month_year_id),
				'fields' => array('DISTINCT Timetable.course_mapping_id'),
				'contain' => array(
						'CourseMapping' => array('fields' => array('CourseMapping.batch_id')),
						)
				)
		); */
		//pr($ttResult);
		/* foreach ($ttResult as $key => $val) {
			$batchesArray[$val['CourseMapping']['batch_id']] = $val['CourseMapping']['batch_id'];
		}
		ksort($batchesArray); */
		//pr($batchesArray);
		...
		$filterCondition="";
		$filterCondition.= "`(CourseMapping`.`batch_id` = $batch_id) AND ";
		//$filterCondition.= "`(CourseMapping`.`program_id` = 3) AND ";
		$filterCondition.= "`(CourseMapping`.`indicator` = 0)";
		if ($month_year_id>0) {
		...
		foreach ($programResult as $key => $pgmResult) {
			...
			foreach ($csmResults as $student_id => $stuid) {
				...
				//echo $csmCount;
				//pr();
				if ($month_year_id==0 || $month_year_id==NULL) {
					$passCount = $this->StudentMark->query("SELECT count(*) as pass_count
				...
				if ($passCount >= $csmCount) { 
					$allPass++;
					//echo "if ".$allPass." ".$program_id." ** StudentID ** ".$student_id." ".$passCount."</br>";
					//echo $student_id."</br>";
				} 
				//else {
				if ($month_year_id==0 || $month_year_id==NULL) {
					...
					$failCount = $this->StudentMark->find('count', array(
						'conditions'=>array('StudentMark.student_id'=>$student_id, 'StudentMark.month_year_id'=>$month_year_id,
								'((StudentMark.status = "Fail" AND StudentMark.revaluation_status=0) OR 
								(StudentMark.revaluation_status=1 AND StudentMark.final_status = "Fail"))',
								'StudentMark.course_mapping_id'=>$cmIdList
						)
					));
				}
			...
			//echo " One : ".$oneArrear." Two : ".$twoArrear." Three : ".$threeArrear." moreThanThreeArrear : ".$moreThanThreeArrear."</br>";
			}
	...
	public function listStudents($batch_id, $program_id, $option, $month_year_id) {
		...
		//pr($csmResults); 
		$stuArray = implode(",", array_keys($csmResults));
		
		$failedArray = $this->StudentMark->query("
				SELECT distinct(student_id) FROM student_marks sm JOIN students s ON sm.student_id=s.id WHERE 
				sm.student_id IN ($stuArray) AND ((sm.status='Fail' AND sm.revaluation_status=0) OR 
				(sm.final_status='Fail' AND sm.revaluation_status=1))  GROUP BY sm.course_mapping_id 
				order by s.id asc
				");
		//pr($failedArray);
		$failedStudents = array();
		foreach ($failedArray as $key => $array) {
			$failedStudents[$array['sm']['student_id']] = $array['sm']['student_id'];
		}
		pr($failedStudents);
		$failedStudentsArray = implode(",", array_keys($failedStudents));
		
		if ($month_year_id==0 || $month_year_id==NULL) {
			$resArray = $this->StudentMark->query("
			...
			StudentMark.marks, StudentMark.final_marks, CourseMapping.month_year_id
			...
			AND StudentMark.month_year_id <= $month_year_id AND StudentMark.student_id IN (".$failedStudentsArray.")
			...
			IN (SELECT max( id ) FROM student_marks sm1 WHERE StudentMark.student_id = sm1.student_id AND
			sm1.month_year_id = $month_year_id GROUP BY sm1.student_id, sm1.course_mapping_id ORDER BY sm1.id DESC) 
			AND ((StudentMark.status='Fail' AND StudentMark.revaluation_status=0) OR (StudentMark.final_status='Fail' 
					AND StudentMark.revaluation_status=1)
			AND StudentMark.month_year_id = $month_year_id AND StudentMark.student_id IN (".$failedStudentsArray.")
			AND Student.discontinued_status=0 
			");
			$dbo = $this->CourseMapping->getDatasource();
