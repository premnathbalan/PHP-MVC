 ...
 */
class CaePractical extends AppModel {
 ...
 */
	...
	public function checkIfCaesExists($courseMappingArray) {
		...
		foreach ($courseMappingArray as $cmId => $courseCode) {
			$caeExists = $this->find('all', array(
					'conditions'=>array(
							'CaePractical.course_mapping_id' => $cmId,
							'CaePractical.assessment_type' => 'CAE',
					),
					'fields' => array('CaePractical.*'),
					'recursive' => 0
			));
	...
	public function manipulateCaeExists($caeExists) {
		...
		foreach ($caeExists as $key => $caePractical) {
			$caeCount = $caeCount+1;
			if($caePractical['CaePractical']['marks_status'] == "Entered") {
				$marks_status = $marks_status+1;
			}
			if($caePractical['CaePractical']['approval_status'] == 1) {
				$approval_status = $approval_status+1;
			}
			if($caePractical['CaePractical']['add_status'] == 1) {
