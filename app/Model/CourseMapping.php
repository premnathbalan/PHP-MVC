...
class CourseMapping extends AppModel {
	...
	public function getBatchAcademicProgramFromCmId($cm_id) {
		$results = $this->find('all', array(
				...
				'contain' => array(
						'Course' => array(
								'fields'=>array('Course.course_code', 'Course.course_name', 'Course.course_type_id'),
	...
	public function listCoursesWithRegNoAndSemesterId($regNo, $semester) {
		$studentDetails = $this->Student->getBatchAndProgramIdFromStudentRegNo($regNo);
		//pr($studentDetails);
		$batch_id = $studentDetails[$regNo]['batch_id'];
		$program_id = $studentDetails[$regNo]['program_id'];
		$student_id = $studentDetails[$regNo]['id'];
		$results = $this->find('all', array(
				'conditions' => array (
						'CourseMapping.batch_id' => $batch_id, 
						'CourseMapping.indicator' => 0,
						'CourseMapping.program_id' => $program_id,
						'CourseMapping.semester_id' => $semester,
				),
				'fields' => array('CourseMapping.id'),
				'contain' => array(
						'Course' => array(
								'fields'=>array('Course.course_code', 'Course.course_name', 'Course.id', 'Course.course_type_id')
						),
						'CourseStudentMapping'=>array(
