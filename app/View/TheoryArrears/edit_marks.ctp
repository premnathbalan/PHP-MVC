...
echo $this->Form->input('cm_id', array('type'=> 'hidden', 'label'=>false, 'default'=>$cm_id, 'name' => 'data[Practical][course_mapping_id]', 'id'=>'cm_id'));
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="margin-top:10px;">
	...
	<?php 
	//pr($results);
	$i =1; foreach ($results as $key => $array) { 
	$student_id = $array[$model]['student_id'];
	$caeArray = $array[$model]['InternalExam'];
	...
	<tr class=" gradeX">
		...
		<td>
			<?php
				echo $this->Form->input('cae_old_marks', array('type'=> 'hidden','id'=>'CaeOldMark'.$i, 'value'=>$cae_mark, 'class'=>'dummy','style'=>'position:relative;margin-top:-14px;width:50px;border-color:#000;color:#000;padding-left:10px;', 'label'=>false, 'name' => 'data[InternalExam][cae_old_marks]['.$student_id.']'));
				echo $this->Form->input('cae_new_marks', array('type'=> 'text','id'=>'CaeNewMark'.$i, 'value'=>$cae_mark, 'class'=>'dummy','style'=>'position:relative;margin-top:-14px;width:50px;border-color:#000;color:#000;padding-left:10px;', 'label'=>false, 'name' => 'data[InternalExam][cae_new_marks]['.$student_id.']', 'onblur'=>"ArrearTheoryCAEEntry($i,this.value,$student_id,$exam_month_year_id);"));
			?>
		</td>
	</tr>
	<?php $i++;} ?>
...
<script>leftMenuSelection('TheoryArrears/theory');</script>

<span class='breadcrumb1'>
<span class='navbar-brand'><small>THEORY <i class='ace-icon fa fa-angle-double-right'></i></small></span>
<?php echo $this->Html->link("<span class='navbar-brand'><small>Arrear Mark Entry <i class='ace-icon fa fa-angle-double-right'></i></small></span>", array("controller"=>"TheoryArrears",'action' => 'theory'),array('data-placement'=>'left','escape' => false)); ?>
</span>