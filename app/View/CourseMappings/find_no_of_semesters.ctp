...
for ($i=1; $i<= $programSemesters; $i++) {
	...
	if(isset($arr[$i])){
		...
		if($this->Html->checkPathAccesstopath('CourseMappings/add','',$authUser['id'])){
			echo $this->Form->button('<i class="ace-icon fa fa-check bigger-110"></i>'.__('Submit'),array('style'=>'float:right;','type'=>'button','name'=>'Save'.$i,'id'=>'Save'.$i,'value'=>'Save','onclick'=>"saveCourseMapping($batchId,$programId,$i);",'class'=>'btn',$btnDisabled));
		}
		echo "</div>";
		echo $this->Form->end();
		echo "</div>";
