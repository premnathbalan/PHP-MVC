					...
					<tr class=" gradeX">
						...
						<?php if ($practical_count > 0) {
								//if($this->Html->checkPathAccesstopath('Arrears/arrear','',$authUser['id'])){
									echo $this->Html->link("<i class='fa fa-pencil fa-lg'></i>",array("controller"=>"Arrears",'action' => 'editPracticalMarks', $cm_id, $exam_month_year_id, $ese_practical_id), array('escape' => false,'style'=>'padding-left:10px;'));
								//}
							}
							else {
								if ($attendance_count > 0) {
									//if($this->Html->checkPathAccesstopath('Arrears/arrear','',$authUser['id'])){
										echo $this->Html->link("<i class='fa fa-plus fa-lg'></i>",array("controller"=>"Arrears",'action' => 'addPracticalMarks', $cm_id, $exam_month_year_id, $ese_practical_id), array('escape' => false,'style'=>'padding-left:10px;'));
									//}
								} else {
									//if($this->Html->checkPathAccesstopath('Arrears/arrear','',$authUser['id'])){
										echo $this->Html->link("<i class='fa fa-plus fa-lg'></i>",array("controller"=>"Arrears",'action' => 'addPracticalMarks', $cm_id, $exam_month_year_id, $ese_practical_id), array('escape' => false,'style'=>'padding-left:10px;'));
