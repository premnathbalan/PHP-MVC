...
echo $this->Form->input('btfou', array('type'=>'hidden', 'label'=>false, 'value'=>'5', 'name'=>'data[btfou]'));
echo $this->Form->input('uid', array('type'=>'hidden', 'label'=>false, 'value'=>'2', 'name'=>'data[uid]'));
...
<table style="width:100%;align:center;border-color:#000;font-weight:14px;margin-top:10px;" border="1" class=" tblOddEven">
	...
	<tr>
		<td align='center'>
			<table>
				<tr>
					<td rowspan='3' rowspan='3' height='24px;'><?php echo $results[0]['UniversityReference']['university_name'];?></td>
				</tr>
			</table>
		</td>
		<td align='center'>
...
$type = '';
	...
	if (isset($courseMapping['CourseStudentMapping'][0]['new_semester_id'])) {
		...
		$type = $courseMapping['CourseStudentMapping'][0]['type'];
	}
	
	$max_cae_mark = $courseMapping['Course']['max_cae_mark'];
	$max_ese_mark = $courseMapping['Course']['max_ese_mark'];
...
SWITCH ($course_type_id) {
	CASE 1:
		$markArray = $this->Html->getTheoryCaeAndEse($course_type_id, $cm_id, $student_id, '-', 0, 0);
		break;
	CASE 2:
	CASE 6:
		$markArray = $this->Html->getPracticalCaeAndEse($course_type_id, $cm_id, $student_id, '-', 0, 0);
		break;
}

$cae_marks=$markArray['cae'];
$ese_marks=$markArray['ese'];
$total = $markArray['total'];
$status = $markArray['status'];
//pr($markArray);
...
/*if(isset($courseMapping['CourseStudentMapping'][0]['Student']['StudentMark'])) { 
	...
	echo "<td>".$this->Form->input('month_year_id'.$cm_id, array('type'=>'select', 'options'=>$monthyears, 'empty'=>'--Select--', 'label'=>false, 'name'=>'data[StudentMark][month_year_id]['.$cm_id.']', 'default'=>$csm_my_id, 'class'=>'month_year_id'))."</td>";
	if($type=="BTFOU") $checked = "checked";
	else $checked="";
	echo "<td>".$this->Form->input('btfou', array('type'=>'checkbox', 'class'=>'btfou', 'label'=>false, $checked, 'style'=>'margin-top:-10px;margin-left:25px;'))."</td>";
	echo "<td align='center'>";
		echo $this->Form->input('cae_marks'.$cm_id, array('type'=>'text', 'name'=>'data[StudentMark][cae_marks]['.$cm_id.']', 'label'=>false, 'value'=>$cae_marks,'style'=>'width:50px;', 'onkeyup'=>"computeBatchTransfer('$cm_id', 'cae', this.value, '$max_cae_mark')"));
	...
	echo "</td>";
	echo "<td align='center'>";
		echo $this->Form->input('ese_marks'.$cm_id, array('type'=>'text', 'label'=>false, 'value'=>$ese_marks, 'name'=>'data[StudentMark][ese_marks]['.$cm_id.']','style'=>'width:50px;', 'onkeyup'=>"computeBatchTransfer('$cm_id', 'ese', this.value, '$max_ese_mark')"));
...
echo $this->Form->button('<i class="ace-icon fa fa-check bigger-110"></i>'.__('Confirm'),array('type'=>'button','name'=>'Confirm','value'=>'Confirm','class'=>'btn',"onclick"=>"return confirmTransferMarks('$reg_number');"));
?>
</div>
