<div>
	...
	if($results){ 
		if($printMode != 'PRINT'){?>	
			...
			<?php //echo $this->Html->link('<i class="ace-icon fa fa-file-excel-o"></i>'.'Excel',array("controller"=>"Students",'action'=>'beforeRevaluationSearch',$examMonth,$batchId,$Academic,$programId,$withheldType,$withheldVal,'Dept Excel', $revalMode),array('type'=>'submit','name'=>'Excel','value'=>'Excel','class'=>'btn','escape' => false,'style'=>'float:right;'));?>
...
<div style="margin-top:5px;width:100%;font-family:'Open sans' !important;font-size:13px;">
 ...
 //if ($student_id==561) pr($result);
		...
		if(($withheldType == 'All') ){
			...
			//if ($result[$p]['attendance'] == 1 && $result[$p]['status'] == "Fail") $cntFail++;
				...
				$courseMark .= "<div style='width:30px;float:left;'>";
					...
					else if ($result[$p]['ese']=="") $result[$p]['ese']="-";
			...
			if (in_array($result[0]['student_id'], array_keys($withheldStudents))) {
				...
				//Newly added to display the marks for withheld students
			...
			//if ($result[$p]['attendance'] == 1 && $result[$p]['status'] == "Fail") $cntFail++;
				...
				//Newly added to display marks ends here 
	...
	<div style='width:90px;float:left;height:30px;'><b><?php echo $result[0]['student_id']." ".$registration_number;?></b></div>
