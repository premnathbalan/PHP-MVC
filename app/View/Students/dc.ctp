...
//pr($courseMappingMandatory);
	...
	//echo "Withdrawal : ".$withdrawal;
		...
		//echo "courseSemester".count($courseSemester);
		//pr($courseSemester);
		
		//echo "semesterIdArray".count($semesterIdArray);
		//pr($semesterIdArray);
		...
		//if (empty($diff_array) && $totalCreditsGained >= $programArray[$programId]) {
			...
			foreach($semIdArray as $key=>$val){
				$htmlData = "";$seqCnt++;
				$sem = $this->Html->retrieveSemesterFromMonthYear($val, $batchId, $programId);
				$htmlData .= ${'courseMark'.$sem};
				//echo $htmlData."</br></br>";
  					...
  					<tr>  						
	  					<td>
	  						<table class="dc13" border="0" width="100%">
						  					...
						  					<tr>
							  					<td style=" vertical-align: middle;" rowspan="3" colspan="2">
							  						<table border="0">
						  					...
						  					<tr>
						  						<td></td>
						  						<td colspan="2" height="50px;" align="right" class="dc20">N[g;gpahu; efu;>  nrd;id-600119></br> jkpo;ehL> ,e;jpah.</td>
						  					</tr>					  					
...
scored in each subject, etc.,) can be viewed in a tabulated format by downloading the "Barcode Reader Software" from www.sathyabamuniversity.ac.in/download.php 2D Barcode scanner is to be used to read the Barcode for verification of marks secured by the candidate.</td>
  		...
  		<?php asort($semIdArray);foreach($semIdArray as $key=>$val){?>
...
function getArrayValues($array) {
	...
	$tmpArray['courseSemester'] = $array['CourseMapping']['month_year_id'];
